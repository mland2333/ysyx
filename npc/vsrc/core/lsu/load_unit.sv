module ysyx_24110006_LOAD_UNIT (
  ...
  always_ff @(posedge i_clock) begin
    ...
    else if (i_vr.valid && i_vr.ready && !check.hit) i_vr.ready <= 0;
    else if (o_rq.valid && !i_vr.ready) i_vr.ready <= 1;
  end
  always_ff @(posedge i_clock) begin
    if (i_reset) o_rq.rq <= 0;
    else if (!o_rq.rq && i_vr.valid && i_vr.ready && !check.hit) o_rq.rq <= 1;
  ...
  logic commit_valid;
  always_ff@(posedge i_clock)begin
    if(i_reset) commit_valid <= 0 ;
    else if(o_rq.valid || i_vr.valid && i_vr.ready && check.hit) commit_valid <= 1;
  ...
  always_ff @(posedge i_clock) begin
    if (o_rq.valid || i_vr.valid && i_vr.ready && check.hit) load_result <= rdata_aligned;
  end
  assign o_rq.addr = rq.addr;
