...
`endif
  ...
  logic in_fencei;
  always_ff@(posedge i_clock)begin
    if(i_reset) in_fencei <= 0;
    else if(i_fencei) in_fencei <= 1;
    else if(in_fencei && o_icache_rq.flush_fin && i_dcache_fencei_fin) in_flush <= 0;
  end
  always_ff @(posedge i_clock) begin
    if (i_reset) pc <= PC;
  ...
  always_ff @(posedge i_clock) begin
    if (i_reset || in_flush || in_fencei) o_icache_rq.rq <= 0;
