module BHT #(
  ...
  always @(posedge i_clock) begin
    for (int i = 0; i < NUM; i++) begin
      if (i_reset) bht[i] <= 0;
      else if (update.valid && hash(update.pc) == i) bht[i] <= {bht[i][INDEX-2:0], update.taken};
    end
  end

  assign i_rq.index   = bht[hash(i_rq.pc)] ^ i_rq.pc[INDEX+3:4];
  assign update_index = bht[hash(update.pc)] ^ update.pc[INDEX+3:4];
endmodule
