module ysyx_24110006_BYPASS(
  ...
  input [31:0] int_result [2],
  input [31:0] lsu_result,
  output bypass::src_t src
);

assign src.d[0] = {32{(loc.loc[0][bypass::from_int0])}} & int_result[0] |
                  {32{(loc.loc[0][bypass::from_int1])}} & int_result[1] |
                  {32{(loc.loc[0][bypass::from_lsu])}} & lsu_result |
                  {32{(loc.loc[0][bypass::from_reg])}} & reg_rdata.d[0];
assign src.d[1] = {32{(loc.loc[1][bypass::from_int0])}} & int_result[0] |
                  {32{(loc.loc[1][bypass::from_int1])}} & int_result[1] |
