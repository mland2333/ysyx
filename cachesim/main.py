...
# 参数设置
num_blocks_list = [2, 4, 8, 16]  # NUM_BLOCKS 参数值
num_ways_list = [1, 2]       # NUM_WAYS 参数值
...
def run_simulation(num_blocks, num_ways, data_width, itrace_file):
    try:
        ...
        return num_blocks, num_ways, data_width, f"Error: {str(e)}", 0, 0, 0
...
def main():
    ...
    for num_blocks, num_ways, data_width, hit_rate, hit, miss, amat in result_list:
        print(f"{num_blocks:^11} | {num_ways:^8} | {data_width:^10} | {hit_rate:^8} | {hit:^11} | {miss:^12} | {amat:^6}")
